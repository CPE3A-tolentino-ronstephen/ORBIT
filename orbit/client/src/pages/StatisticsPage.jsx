...
function TopNButtons({ topN, setTopN }) {
  return (
...
export default function StatisticsPage() {
  ...
  const trendData    = isCovid ? (yearlyHistorical ?? []) : historical;
  const trendLoading = isCovid ? yearlyLoading : loading;
  ...
  const topByDeaths = useMemo(
    () => [...countries].filter(c => c.deaths > 0).sort((a, b) => b.deaths - a.deaths).slice(0, topN),
    [countries, topN]
  );
  ...
  const globalResolved = useMemo(() => {
    if (!isOwid || !historical.length) return global;
    const last = historical[historical.length - 1];
    return {
      ...global,
      cases:  last.cases  ?? global?.cases,
      deaths: last.deaths ?? global?.deaths,
    };
  }, [global, historical, isOwid]);

  const kpis = useMemo(() => {
    const src = globalResolved ?? global;
    if (!src) return {};
    return {
      cases:       fmt(src.cases),
      deaths:      fmt(src.deaths),
      cfr:         isHiv
        ? Number(src.caseFatalityRate).toFixed(2)
        : fmtPct(src.caseFatalityRate),
      recovered:   fmt(src.recovered),
      active:      fmt(src.active),
      critical:    fmt(src.critical),
  ...
  return (
    <div className="stats-page page-enter">
      <style>{`
        ...
        .stats-tabs { display: flex; gap: .6rem; flex-wrap: wrap; align-items: center; }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: ".5rem", marginTop: ".2rem" }}>
            ...
            {!loading && (
              <span className="data-badge">
                {isCovid
                  ? `disease.sh · updated ${kpis.updatedDate ?? "—"}`
                  : isOwid
                    ? `OWID / WHO · ${kpis.year ?? "annual"}`
                    : "disease.sh · real-time"}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="kpi-row">
        {isHiv ? (
          <>
            ...
            <div className="kpi-card">
              <div className="kpi-label">Infection-to-Death Ratio</div>
              {loading ? <div className="kpi-skeleton" /> : <div className="kpi-val">{kpis.cfr ?? "—"}</div>}
              <div className="kpi-sub">new infections / deaths</div>
            </div>
            <div className="kpi-card">
          ...
          <>
            ...
            {!isCovid && (
              <div className="kpi-card">
                <div className="kpi-label">{rateLabel}</div>
                {loading ? <div className="kpi-skeleton" /> : <div className="kpi-val">{kpis.cfr ?? "—"}</div>}
            ...
            {!isOwid && isCovid && [
              {
                label: "Recovery Rate",
                val: global?.cases > 0 ? fmtPct((global.recovered / global.cases) * 100) : "—",
                sub: "recovered / cases",
              },
              { label: "Critical", val: kpis.critical, sub: "serious condition" },
            ].map(({ label, val, sub }) => (
              <div className="kpi-card" key={label}>
                <div className="kpi-label">{label}</div>
                {loading ? <div className="kpi-skeleton" /> : <div className="kpi-val">{val ?? "—"}</div>}
      ...
      <div className="charts-grid">
        ...
        <div className="chart-card chart-span">
          <div className="chart-title">{trendTitle}</div>
          {trendLoading ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={trendData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                ...
                <Area
                  type="monotone"
                  dataKey="cases"
                  name={isHiv ? "New Infections" : "Cases"}
                  stroke="#3b82f6"
                  strokeWidth={2}
                  fill="url(#gCases)"
                  dot={isMpox}
                ...
                <Area
                  type="monotone"
                  dataKey="deaths"
                  name={isHiv ? "Annual Deaths" : "Deaths"}
                  stroke="#ef4444"
                  strokeWidth={2}
                  fill="url(#gDeaths)"
                  dot={isMpox}
        ...
        <div className="chart-card">
          <div className="chart-title">
            ...
            <TopNButtons topN={topN} setTopN={setTopN} />
          </div>
          {loading ? (
            <NoData icon="" text="Loading..." />
          ) : topCountries.length === 0 ? (
            <NoData icon="" text="No country data loaded" />
          ) : (
            <ResponsiveContainer width="100%" height={260}>
        ...
        {/* Second bar chart: mortality / deaths / CFR depending on disease */}
        <div className="chart-card">
          {isOwid && !isHiv ? (
            <>
              <div className="chart-title">
              ...
              {loading ? (
                <NoData icon="" text="Loading..." />
              ) : topByMortality.length === 0 ? (
                <NoData icon="" text="No data available" />
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={topByMortality} layout="vertical" margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Tooltip content={<CustomTooltip />} />
                    <Bar dataKey="caseFatalityRate" name={rateLabel} fill="#ef4444" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </>
            ...
            <>
              ...
              {loading ? (
                <NoData icon="" text="Loading..." />
              ) : topByDeaths.length === 0 ? (
                <NoData icon="" text="No data available" />
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={topByDeaths} layout="vertical" margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" horizontal={false} />
                    <XAxis
                      type="number"
                      tickFormatter={tickFmt}
        ...
        {/* Continent pie - full width, pie left / legend right */}
        {!isHiv && (
          <div className="chart-card chart-span">
            ...
            {loading ? (
              ...
              <NoData icon="" text="No continent data available" />
            ) : (
              <div className="pie-layout">
                <div style={{ flex: "0 0 auto" }}>
                  <ResponsiveContainer width={320} height={300}>
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={v => fmt(v)}
                        contentStyle={{ fontFamily: "var(--font-body)", fontSize: ".82rem" }}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="pie-legend">
                  {continentPie.map(({ name, value }) => (
                    <div key={name} className="pie-legend-row">
                      <span
                        className="pie-legend-dot"
                        style={{ background: CONTINENT_COLORS[name] ?? CONTINENT_COLORS.Other }}
                      />
                      <span className="pie-legend-name">{name}</span>
              ...
              <div className="risk-dist-row" key={label}>
                ...
                <div className="risk-dist-bar">
                  <div
                    ...
                    style={{
                      width: loading ? "0%" : `${(count / maxRisk) * 100}%`,
                      background: color,
                    }}
                  />
                </div>
