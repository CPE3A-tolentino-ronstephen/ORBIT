...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      padding:".75rem 1rem", boxShadow:"0 4px 16px rgba(0,0,0,.1)",
...
function NoData({ icon = "", text = "No data available" }) {
  return (
    <div style={{
      ...
      height:200, color:"var(--gray-400)", fontSize:".875rem",
      textAlign:"center", flexDirection:"column", gap:".5rem",
    }}>
  ...
  return (
    <div className="stats-page page-enter">
      <style>{`
        ...
        .stats-header { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1.25rem; padding-bottom:.5rem; border-bottom:1px solid var(--border); }
        .stats-title { font-size:1.4rem;font-weight:800;color:var(--gray-900);letter-spacing:-.02em; }
        .stats-tabs { display:flex; gap:.4rem; flex-wrap:wrap; }.stats-tabs { display:flex; gap:.6rem; flex-wrap:wrap; align-items:center; }
        .stats-tab { padding:.45rem 1.1rem;border-radius:99px;font-size:.82rem;font-weight:600;cursor:pointer;border:1.5px solid var(--border);background:var(--white);color:var(--gray-600);transition:all .18s;font-family:var(--font-body); white-space:nowrap; }
        ...
        @media (max-width:900px) { .charts-grid { grid-column:1fr; } }
      ...
      <div className="kpi-row">
        {isHiv ? (
          ...
          <>
            ...
            {!isOwid && !isCovid === false && [
              { label:"Recovery Rate", val: global?.cases > 0 ? fmtPct((global.recovered / global.cases) * 100) : "—", sub:"recovered / cases" },
            ...
            {riskBands.map(({ label, color, count }) => (
              <div className="risk-dist-row" key={label}>
                ...
                <div className="risk-dist-bar">
                  <div className="risk-dist-fill" style={{ width: loading ? "0%" : `${(count / maxRisk) * 100}%`, background: color }} />
                </div>
